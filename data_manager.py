...
def get_question_by_id(cursor: RealDictCursor, question_id: int) -> list:
    query = f"""
        SELECT question.*, forum_user.id, forum_user.mail, forum_user.reputation
        FROM question 
...
def get_answers_by_question_id(cursor: RealDictCursor, question_id: int) -> list:
    query = f"""
        SELECT answer.*, forum_user.id, forum_user.mail, forum_user.reputation
        FROM answer
...
def get_comments_by_question_id(cursor: RealDictCursor, question_id: int):
    query = f"""
            SELECT comment.*, forum_user.id, forum_user.mail,forum_user.reputation
            FROM comment
